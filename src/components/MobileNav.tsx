    ...
    return (
        <nav className="fixed top-0 left-0 right-0 flex w-full h-16 bg-background/80 backdrop-blur-md lg:hidden">
            <div className="relative flex items-center justify-between w-full px-5">
                ...
                <div className="relative flex z-[99999] items-center gap-2">
                    <ThemeToggle />
                    <Hamburger
                        toggled={isOpen}
                        toggle={setIsOpen}
                ...
                <AnimatePresence presenceAffectsLayout>
                    {isOpen && (
                        <motion.div
                            ...
                            className="fixed z-[9999] rounded-lg w-full h-screen inset-0 backdrop-blur-md bg-background"
                        >
                            <ul className="flex flex-col items-start w-full px-6 py-3 pt-16 space-y-3">
                                {links?.map((link, index) => (
                                    <motion.li
                                        ...
                                        className="w-full px-4 py-2 text-lg font-normal text-foreground transition transform rounded-md cursor-pointer text-start active:scale-95 hover:bg-muted active:opacity-80"
                                    >
